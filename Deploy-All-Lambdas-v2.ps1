...
# Event source mapping SQS (debe existir de Semana 1)
Write-Host "  🔗 Verificando trigger SQS..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0 -and $existingMappings -and -not [string]::IsNullOrWhiteSpace($existingMappings) -and -not $existingMappings.Contains("None")) {
    Write-Host "  ✓ Trigger SQS ya configurado" -ForegroundColor Green
} else {
    Write-Host "  ⚠️ Configurando trigger SQS..." -ForegroundColor Yellow
