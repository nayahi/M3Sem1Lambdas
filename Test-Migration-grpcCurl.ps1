...
####################
# ─────────────────────────────────────────────────────────────────
# FUNCIÓN: Crear una orden con saga vía grpcurl (directo a OrderService)
# ─────────────────────────────────────────────────────────────────
function New-OrderWithSaga {
    ...
    $payload = @{
        user_id = 2
        shipping_address = "Test Address $OrderNumber, San Jose, Costa Rica"
        items = @(
            @{
                product_id = 1
                quantity   = 1
    ...
    try {
        ...
        $response = $payload | grpcurl -plaintext -d @ `
            $GrpcHost orderservice.OrderService/CreateOrderWithSaga 2>&1

        $stopwatch.Stop()

        if ($LASTEXITCODE -eq 0) {
            # Extraer status del JSON de respuesta
            ...
            Write-Host "  ✅ Orden #$OrderNumber: ID=$orderId, Status=$status, Tiempo=$($stopwatch.ElapsedMilliseconds)ms" -ForegroundColor Green
        } else {
            Write-Host "  ❌ Orden #$OrderNumber falló: $response" -ForegroundColor Red
        }

        return $response
    }
    catch {
        Write-Host "  ❌ Orden #$OrderNumber error: $($_.Exception.Message)" -ForegroundColor Red
        return $null
    }
}

# ─────────────────────────────────────────────────────────────────
# FUNCIÓN: Crear N órdenes y mostrar resumen
# ─────────────────────────────────────────────────────────────────
function New-MultipleOrders {
    ...
    for ($i = 1; $i -le $Count; $i++) {
        $result = New-OrderWithSaga -OrderNumber $i -GrpcHost $GrpcHost
        if ($LASTEXITCODE -eq 0) { $successful++ } else { $failed++ }
        # Pausa para que los logs del OrderService sean legibles
    ...
    Write-Host "      'Facade→gRPC'     = Ruta original" -ForegroundColor Gray
    Write-Host "      'Facade→SQS'      = Ruta Lambda (migrada)" -ForegroundColor Gray
